// plugins/ircd/check_ircd.cpp
// IRC daemon monitoring plugin
...
class IrcdPlugin : public netmon_plugins::Plugin {
private:
    std::string hostname;
    int port = 6667;
    int timeoutSeconds = 10;
    std::string nickname = "monitor";
    ...
    bool checkIrcd(const std::string& host, int portNum, int timeout, const std::string& nick) {
#ifdef _WIN32
        WSADATA wsaData;
        ...
        // Send USER command
        if (!sendCommand(sock, "USER " + nick + " 0 * :Monitoring Bot")) {
            closesocket(sock);
            WSACleanup();
            return false;
        }
        
        // Read response (should get welcome message)
        response = readResponse(sock);
        bool success = (response.find("001") != std::string::npos || 
                       response.find("433") != std::string::npos); // Nick in use is also OK
...
public:
    netmon_plugins::PluginResult check() override {
        ...
        try {
            bool available = checkIrcd(hostname, port, timeoutSeconds, nickname);
            
            if (available) {
                std::ostringstream msg;
                msg << "IRC OK - " << hostname << ":" << port << " is accepting connections";
                return netmon_plugins::PluginResult(
                ...
                std::ostringstream msg;
                msg << "IRC CRITICAL - " << hostname << ":" << port << " is not accepting connections";
                return netmon_plugins::PluginResult(
            ...
            return netmon_plugins::PluginResult(
                netmon_plugins::ExitCode::UNKNOWN,
                "IRC check failed: " + std::string(e.what())
            );
        }
    }
    ...
    void parseArguments(int argc, char* argv[]) override {
        for (int i = 1; i < argc; i++) {
            ...
            } else if (strcmp(argv[i], "-n") == 0 || strcmp(argv[i], "--nickname") == 0) {
                if (i + 1 < argc) {
                    nickname = argv[++i];
                }
            }
    ...
    std::string getUsage() const override {
        return "Usage: check_ircd -H <hostname> [options]\n"
               "Options:\n"
               "  -H, --hostname HOST     Hostname or IP address\n"
               "  -p, --port PORT         IRC port (default: 6667)\n"
               "  -t, --timeout SECONDS   Timeout in seconds (default: 10)\n"
               "  -n, --nickname NICK     IRC nickname to use (default: monitor)\n"
               "  -h, --help              Show this help message";
    }
    
    std::string getDescription() const override {
        return "Monitor IRC daemon service availability";
    }
};
