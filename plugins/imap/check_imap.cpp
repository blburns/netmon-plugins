...
class ImapPlugin : public netmon_plugins::Plugin {
private:
    std::string hostname;
    int port = 143;
    ...
    bool checkImap(const std::string& host, int portNum, int timeout, 
        ...
        // Send LOGIN command if credentials provided
        if (!user.empty() && !pass.empty()) {
            if (!sendCommand(sock, "A1 LOGIN " + user + " " + pass)) {
                closesocket(sock);
                WSACleanup();
                return false;
            }
            ...
            if (response.empty() || response.find("A1 OK") == std::string::npos) {
                closesocket(sock);
                WSACleanup();
                return false;
            }
...
public:
    netmon_plugins::PluginResult check() override {
        ...
        try {
            bool available = checkImap(hostname, port, timeoutSeconds, username, password);
            
            if (available) {
                std::ostringstream msg;
                msg << "IMAP OK - " << hostname << ":" << port << " is accepting connections";
                return netmon_plugins::PluginResult(
                ...
                std::ostringstream msg;
                msg << "IMAP CRITICAL - " << hostname << ":" << port << " is not accepting connections";
                return netmon_plugins::PluginResult(
            ...
            return netmon_plugins::PluginResult(
                netmon_plugins::ExitCode::UNKNOWN,
                "IMAP check failed: " + std::string(e.what())
            );
        }
    }
    ...
    void parseArguments(int argc, char* argv[]) override {
        for (int i = 1; i < argc; i++) {
            if (strcmp(argv[i], "-h") == 0 || strcmp(argv[i], "--help") == 0) {
                ...
                useSSL = true;
                if (port == 143) port = 993; // Default IMAPS port
            }
        }
    }
    
    std::string getUsage() const override {
        return "Usage: check_imap -H <hostname> [options]\n"
               "Options:\n"
               "  -H, --hostname HOST     Hostname or IP address\n"
               "  -p, --port PORT         IMAP port (default: 143, 993 for SSL)\n"
               "  -t, --timeout SECONDS   Timeout in seconds (default: 10)\n"
               "  -u, --username USER     Username for authentication\n"
               "  -P, --password PASS     Password for authentication\n"
               "  -S, --ssl               Use SSL/TLS (IMAPS)\n"
               "  -h, --help              Show this help message";
    }
    
    std::string getDescription() const override {
        return "Monitor IMAP service availability";
    }
};
