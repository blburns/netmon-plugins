// plugins/nntp/check_nntp.cpp
// NNTP (Network News Transfer Protocol) service monitoring plugin
...
class NntpPlugin : public netmon_plugins::Plugin {
private:
    std::string hostname;
    int port = 119;
    ...
    bool checkNntp(const std::string& host, int portNum, int timeout, 
        ...
        // Read greeting (should start with 200 or 201)
        std::string response = readResponse(sock);
        if (response.empty() || (response[0] != '2' && response[0] != '4')) {
            closesocket(sock);
            WSACleanup();
            return false;
        }
        ...
        if (!user.empty() && !pass.empty()) {
            if (!sendCommand(sock, "AUTHINFO USER " + user)) {
                closesocket(sock);
                WSACleanup();
                return false;
            }
            ...
            if (!sendCommand(sock, "AUTHINFO PASS " + pass)) {
                closesocket(sock);
                WSACleanup();
                return false;
            }
...
public:
    netmon_plugins::PluginResult check() override {
        ...
        try {
            bool available = checkNntp(hostname, port, timeoutSeconds, username, password);
            
            if (available) {
                std::ostringstream msg;
                msg << "NNTP OK - " << hostname << ":" << port << " is accepting connections";
                return netmon_plugins::PluginResult(
                ...
                std::ostringstream msg;
                msg << "NNTP CRITICAL - " << hostname << ":" << port << " is not accepting connections";
                return netmon_plugins::PluginResult(
            ...
            return netmon_plugins::PluginResult(
                netmon_plugins::ExitCode::UNKNOWN,
                "NNTP check failed: " + std::string(e.what())
            );
        }
    }
    ...
    std::string getUsage() const override {
        return "Usage: check_nntp -H <hostname> [options]\n"
               "Options:\n"
               "  -H, --hostname HOST     Hostname or IP address\n"
               "  -p, --port PORT         NNTP port (default: 119)\n"
    ...
    std::string getDescription() const override {
        return "Monitor NNTP (Network News Transfer Protocol) service availability";
    }
};
